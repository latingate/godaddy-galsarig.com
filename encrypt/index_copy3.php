...
<div class="container">
    ...
    if ($textStringToEncrypt && !empty($userPassword)) {
        ...
        if ($encyptedText) {
            $resultsHTML .= "<label for='EncryptedText'>Encrypted text</label><div class='form-group row'><div class='col-9'><input type='text' class='form-control' id='EncryptedText' value='" . $encyptedText . "'></div><div class='col-3 '><button onclick='copyLink(\"EncryptedText\")'>Copy Link</button></div></div><br/>";
            if ($createShortURL) {
                $bitlyShortURL = creatlyBitlink($_SERVER['SCRIPT_URI'] . '?textToDecrypt=' . $encyptedText);
                $resultsHTML .= "<label for='bitlyShortURL'>bitly Short URL</label><div class='form-group row'><div class='col-3'><input type='text' class='form-control' id='bitlyShortURL' value='" . $bitlyShortURL . "'></div><div class='col-3 '><button onclick='copyLink(\"bitlyShortURL\")'>Copy Link</button></div></div><br/>";
            }
            $results = true;
        }
    }
    ...
    if ($textStringToDecrypt) {
        ...
        if ($decyptedText) {
            ...
            $resultsHTML .= "<h4>Decrypted text:</h4><h5 class='text-danger'>". $decyptedText . "</h5><br/>";
            $results = true;
    ...
    <br/>
    <div class="p-2 mb-2 bg-warning text-center">
    ...
    <form method="POST" action="<?= $_SERVER['SCRIPT_URI'] ?>">
        <div class="form-group">
            ...
            ?>
                ...
                <input type="text" class="form-control" id="textStringToEncrypt" name="textStringToEncrypt" placeholder="Type your secret message here.."
            ...
            <input type="text" class="form-control" id="textStringToDecrypt" name="textStringToDecrypt" placeholder="If you got encrypted text - paste it here"
                   value="<?= $textStringToDecrypt ?>">
            <br/>
            <label for="userPassword">Password key</label>
            <input type="text" size='70' class="form-control" id="userPassword" name="userPassword" placeholder="A password key shared with your partner"
                   value="<?= $userPassword ?>">
            <br/>
            <?php
            if (empty($textStringToDecrypt)) {
            ?>
                <div class="form-check">
                    <input type="checkbox" class="form-check-input" id="createShortURL" name="createShortURL" checked>
